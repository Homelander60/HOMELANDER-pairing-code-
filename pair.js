//copy code? pls put 𝙃𝙊𝙈𝙀𝙇𝘼𝙉𝘿𝙀𝙍 as credit
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +27687077594")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number ?\nFor example: +27687077594 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
//------------------------------------------------------
    XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
            await delay(1000 * 10)
            await XeonBotInc.sendMessage(XeonBotInc.user.id, { text: `Support/Contact Developer\n\n\n⎆Follow: https://www.instagram.com/mirai.art69?igsh=ZXQzOHA4d3J0aGNl\n\n⎆Whatsapp ANIME GC: https://chat.whatsapp.com/Gei7mdybqG8Hf854QUDIdV` });
  ...
  Thanks for using H-PairCode
©2023-2024 𝙃𝙊𝙈𝙀𝙇𝘼𝙉𝘿𝙀𝙍BotInc\n\n ` }, {quoted: xeonses});
              await delay(1000 * 2) 
              process.exit(0)
        }
        if (
